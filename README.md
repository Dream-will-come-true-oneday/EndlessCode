...
- **MCP 工具延迟暴露**：启动时仍完整发现并缓存 MCP 工具目录，但模型初始只看到内置工具和只读 `mcp_search_tools`；搜索命中后，匹配工具的完整 schema 在下一轮请求中出现，激活状态只保留在当前进程。
...
## 本次更新：权限解释审计与 MCP 工具延迟加载

本次更新补齐了长程 Agent 在安全可解释性和大规模工具接入上的两个基础能力：

- **权限解释**：每次工具调用都会生成确定性的风险说明，审批面板展示风险等级、操作目标、命中的规则来源和潜在影响；黑名单命中、路径越界等高危操作标记为 critical 并直接拒绝。
- **权限审计**：按会话写入 `.endless-code/sessions/<会话 ID>/permission-audit.jsonl`，记录权限检查、审批响应、执行开始和执行结束四类事件。参数和结果只保留脱敏摘要，审计写入失败不会阻塞工具执行；空闲时输入 `/audit` 查看最近 50 条记录。
- **MCP 延迟加载**：启动时处理 MCP `tools/list` 的全部分页并在本地建立工具目录，但模型初始只接收内置工具和 `mcp_search_tools`。模型搜索后，匹配工具的完整 schema 在下一轮请求中按需暴露，避免百级 MCP 工具定义持续占用上下文。
