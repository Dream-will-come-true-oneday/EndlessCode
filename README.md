...
- **MCP 工具延迟加载**：启动时仍完整发现并缓存 MCP 工具目录，但系统提示会引导模型先使用 ToolSearch（`mcp_search_tools`）；每轮请求临时列出未加载工具名称，搜索命中后，匹配工具的完整 schema 在下一轮请求中出现，激活状态只保留在当前进程。
...
- **MCP 工具延迟加载**：启动时处理 MCP `tools/list` 的全部分页并在本地建立工具目录，但模型初始只接收内置工具和 `mcp_search_tools`。系统提示明确要求先通过 ToolSearch 查询；每轮请求额外临时列出所有未加载的 MCP 工具名称，ToolSearch 命中后将名称加入客户端的 `activated_tools` 集合，下一轮 `tools` 数组按需带入完整 schema，避免百级 MCP 工具定义持续占用上下文。
