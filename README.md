...
`context_window` 决定压缩时机：所有压缩阈值按窗口等比推导，不再使用固定绝对值。可用窗口先扣除本轮输出预算与工具定义开销，再减去摘要预留与安全余量，得到自动压缩触发点：

```text
自动压缩阈值 = 窗口 − 输出预留 − 工具定义 token − 摘要预留 − 自动安全余量
```

| 限制项 | 比例 | 下限 | 上限 | 200K 窗口 | 1M 窗口 |
|------|------|------|------|------|------|
| 单条工具结果落盘线 | 0.250 | 8,000 | 250,000 | 50,000 B | 250,000 B |
| 同消息聚合落盘线 | 1.000 | 32,000 | 1,000,000 | 200,000 B | 1,000,000 B |
| 摘要预留 | 0.100 | 4,000 | 100,000 | 20,000 | 100,000 |
| 自动安全余量 | 0.065 | 2,000 | 65,000 | 13,000 | 65,000 |
| 手动/紧急安全余量 | 0.015 | 1,000 | 15,000 | 3,000 | 15,000 |
| 摘要后近期原文保留 | 0.050 | 4,000 | 50,000 | 10,000 | 50,000 |
| 恢复附件单文件上限 | 0.025 | 2,000 | 25,000 | 5,000 | 25,000 |
| 本轮输出预留 | 0.040 | 4,096 | 40,000 | 8,000 | 40,000 |

1M 窗口下各项等于历史固定常量（比例表以 1M 为上限校准），因此默认配置行为不变；配成 128K 时自动压缩阈值约为 101,760 token，仍会正常触发。窗口小至容纳不下正常余量时进入降级模式（保留量退到下限继续压缩），`/status` 与状态栏会标注降级。比例表内置于代码，不作为用户配置项；token 估算比例会根据模型回报的真实用量自校准。设计与验收见 [docs/dynamic-context](docs/dynamic-context/spec.md)。

**压缩阶梯（成本从低到高）：** 达到触发线后依次尝试三层，前一层的驻留量已达标就不往后走——

1. **L0 落盘**：超限的工具结果全文写入会话目录，历史里只留头部预览与路径指针。
2. **L1 本地裁剪**（零模型调用）：同一文件被多次读取时，旧读取换成「已有更新版本」指针；距近期较远的落盘预览降级为纯指针（某路径最新一次读取不降级）。已生成的指针会被冻结重放，下一轮不会回退成预览，也不会拆开工具调用与结果的配对。
3. **L2 摘要**：自动压缩采用**增量滚动摘要**——会话目录下的 `summary-state.json` 记录上一版摘要与已覆盖消息数，每次只摘覆盖点之后的新片段并与旧摘要合并，摘要成本不随会话长度线性增长；恢复会话后继续递增。首次压缩、手动 `/compact` 与超限紧急压缩仍为全量重摘（手动与紧急不写滚动状态）。摘要结果要过质量门（驻留量、非空摘要、无孤儿工具结果），不达标会保留原历史并计入熔断。

**窗口自校准：** 配置窗口大于模型实际接受量时，请求超限会把有效窗口收敛为 `min(配置值, 失败时估算 × 0.95)`（下限 16,000），随即重建压缩预算并提示一次；同一会话内多次收敛只取更小的窗口且不再重复提示，`/model` 切换或重启会复位为配置值。`/status` 的「滚动摘要轮次」与「校准后窗口」可直接观察这两项行为。设计与验收见 [docs/compaction-l1-rolling](docs/compaction-l1-rolling/spec.md)。
