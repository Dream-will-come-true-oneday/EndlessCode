...
def test_loader_merges_three_levels_in_priority_order(tmp_path: Path) -> None:
    ...
    config.mkdir()
    user.mkdir()
    (tmp_path / "MEWCODE.md").write_text("root", encoding="utf-8")
    (config / "MEWCODE.md").write_text("config", encoding="utf-8")
    (user / "MEWCODE.md").write_text("user", encoding="utf-8")
...
def test_loader_stops_after_five_levels_and_skips_binary(tmp_path: Path) -> None:
    ...
    (tmp_path / "MEWCODE.md").write_text("@include 1.md", encoding="utf-8")
    assert "超过最大嵌套深度" in Loader(str(tmp_path)).load()

    (tmp_path / "MEWCODE.md").write_text("@include binary.md", encoding="utf-8")
