...
- **F1**：系统启动时按以下顺序扫描三个路径，找到就加载、找不到就跳过：① `<project_root>/MEWCODE.md`（项目级，最高优先级）；② `<project_root>/.endless-code/MEWCODE.md`（项目配置级）；③ `~/.config/endless-code/MEWCODE.md`（用户级，最低优先级）。三份文件的内容按此优先级顺序拼接——高优先级在前，模型优先遵循；各层之间用空行分隔。
