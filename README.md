...
- **MCP 工具延迟加载**：启动时仍完整发现并缓存 MCP 工具目录，但模型初始只看到内置工具和只读 `mcp_search_tools`；搜索命中后，匹配工具的完整 schema 在下一轮请求中出现，激活状态只保留在当前进程。
...
## 本次更新：MCP 工具延迟加载与五层权限拦截

本次更新将两项能力作为 Agent 系统的基础能力：

- **MCP 工具延迟加载**：启动时处理 MCP `tools/list` 的全部分页并在本地建立工具目录，但模型初始只接收内置工具和 `mcp_search_tools`。模型搜索后，匹配工具的完整 schema 在下一轮请求中按需暴露，避免百级 MCP 工具定义持续占用上下文。
- **五层权限拦截**：工具调用依次经过危险命令黑名单、项目路径沙箱、三级权限规则、权限模式和人在回路审批。每次判定同时生成确定性的权限解释，审批面板展示风险等级、操作目标、规则来源和潜在影响；黑名单命中、路径越界等高危操作标记为 critical 并直接拒绝。
- **权限解释审计**：权限解释作为五层拦截的一部分，审计记录按会话写入 `.endless-code/sessions/<会话 ID>/permission-audit.jsonl`，覆盖权限检查、审批响应、执行开始和执行结束四类事件。参数和结果只保留脱敏摘要，审计写入失败不会阻塞工具执行；空闲时输入 `/audit` 查看最近 50 条记录。
- **兼容执行链路**：延迟加载只改变模型看到的工具定义，不改变 MCP `call_tool`、现有权限判断、人在回路审批和工具执行超时行为。激活状态只存在于当前进程，不写入项目文件。

实现采用“本地目录搜索 + 下一轮激活”的边界：MCP 客户端仍需完整发现远端元数据，延迟发生在 Registry 向模型导出 schema 的阶段。默认每次搜索最多激活 5 个工具，可通过 `limit` 控制在 1 到 10 个之间。
...
- MCP `tools/list` 的 `nextCursor` 分页会全部收录到本地目录；模型可调用 `mcp_search_tools({"query":"关键词","limit":5})` 搜索，结果仅包含名称和简短描述。
- 未经搜索激活的 MCP 工具不会出现在模型工具定义中，直接调用会返回“请先使用 `mcp_search_tools`”的引导错误。
...
每次需要判权的工具调用都会生成确定性的权限解释，审批面板会显示风险等级、目标、规则来源和影响说明。调用的判定、审批、执行开始和结束事件按会话追加到 `.endless-code/sessions/<会话 ID>/permission-audit.jsonl`；参数与结果只保留脱敏摘要，写入失败不会阻塞 Agent。空闲时输入 `/audit` 可查看当前会话最近记录。
